...
func (o *otWrapper) Call(ctx context.Context, req client.Request, rsp interface{}, opts ...client.CallOption) error {
	...
	span.LogFields(opentracinglog.String("grpc-client.req", fmt.Sprintf("%+v", req.Body())))
	defer span.Finish()
	if err = o.Client.Call(ctx, req, rsp, opts...); err != nil {
		span.LogFields(opentracinglog.String("error", err.Error()))
		span.SetTag("error", true)
	}
	span.LogFields(opentracinglog.String("grpc-client.rsp", fmt.Sprintf("%+v", rsp)))
	return err
}
...
func NewHandlerWrapper(ot opentracing.Tracer) server.HandlerWrapper {
	return func(h server.HandlerFunc) server.HandlerFunc {
		return func(ctx context.Context, req server.Request, rsp interface{}) error {
			...
			span.LogFields(opentracinglog.String("grpc-server.req", fmt.Sprintf("%+v", req.Body())))
			defer span.Finish()
			if err = h(ctx, req, rsp); err != nil {
				span.LogFields(opentracinglog.String("error", err.Error()))
				span.SetTag("error", true)
			}
			span.LogFields(opentracinglog.String("grpc-server.rsp", fmt.Sprintf("%+v", rsp)))
			return err
		}
	}
}
